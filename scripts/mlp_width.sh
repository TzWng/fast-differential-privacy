...
#       --dimension "$dim" \
#       --optimizer SGD \
#       --log_path "/content/drive/MyDrive/DP_muP/logs/MLP_SGD_depth5_diffwidth_approx_ratio_mup_2_ep20.txt"
#   done
# done
...
LRS=(-7 -6.5 -6 -5.5 -5 -4.5 -4 -3.5 -3) # SGD
# 288 512 1152 2048 3200 4608
for wid in 512; do
  for lr in "${LRS[@]}"; do 
    sig=$(awk "BEGIN {print 2.0*sqrt(128.0/288)}")
    dim=$(awk "BEGIN {print sqrt($wid/128.0)*8.0}")
    echo "Running width=$wid, lr=$lr, noise=$sig, dim=$dim"
    $PYTHON -m scripts.MLP_sp \
      --width "$wid" \
