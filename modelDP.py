...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.flash:
            ...
            y = torch.nn.functional.scaled_dot_product_attention(q, k, v, attn_mask=None, dropout_p=self.dropout if self.training else 0, is_causal=True, scale=1.0 / k.size(-1))
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        pos = torch.arange(0, t, dtype=torch.long, device=device).unsqueeze(dim=0).repeat(b, 1) # shape (t)
